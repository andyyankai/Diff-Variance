...
def mis_weight_ad(pdf_a, pdf_b):
    a2 = dr.sqr(pdf_a)
    b2 = dr.sqr(pdf_b)
    w = a2 / (a2 + b2)
    return dr.select(dr.isfinite(w), w, 0)

class PRBM2Integrator(RBIntegrator):
    r"""
    .. _integrator-prb:
    ...
    def sample(self,
        ...
        while loop(active):
            ...
            prev_emi_pdf = scene.pdf_emitter_direction(prev_si, ds, ~prev_bsdf_delta)

            with dr.resume_grad(when=not primal):
                if first_ray == False:
                    prev_bsdf_ctx = mi.BSDFContext()
                    prev_bsdf = prev_si.bsdf(ray)
                    r_tmp = prev_si.spawn_ray(prev_si.to_world(prev_wo))
                    prev_bsdf_pdf_ad = prev_bsdf.pdf(prev_bsdf_ctx, prev_si, prev_si.to_local(r_tmp.d), prev_active_next)
                else:
                    prev_bsdf_pdf_ad = prev_bsdf_pdf
            ...
            with dr.resume_grad(when=not primal):
                ...
                mis_em = dr.select(ds.delta, 1, mis_weight_ad((ds.pdf), (bsdf_pdf_em)))
                if self.split == "bsdf":
                    mis_em = 0.0
            ...
            bsdf_sample, bsdf_weight = bsdf.sample(bsdf_ctx, si,
                                                   ...
                                                   sampler.next_2d(),
                                                   active_next)
            prev_wo = bsdf_sample.wo
            prev_active_next = active_next
            ...
            if not primal:
                with dr.resume_grad():
                    ...
                    # Detached version of the above term and inverse (M2 is attached)
                    bsdf_val_det = bsdf_weight * bsdf_pdf
                    inv_bsdf_val_det = dr.select(dr.neq(bsdf_val_det, 0),
                                                 dr.rcp(bsdf_val_det), 0)
                    detach_inv_bsdf_val_det = dr.detach(inv_bsdf_val_det)
                    ...
                    Lr_ind_detach = L * dr.replace_grad(1, detach_inv_bsdf_val_det * bsdf_val)


                    # Differentiable Monte Carlo estimate of all contributions
                    if self.m_val == 2:
                        Lo = dr.sqrt((Le + Lr_dir + Lr_ind)*(Le + Lr_dir + Lr_ind_detach))*2.0*L_init
                    else:
                        Lo = Le + Lr_dir + Lr_ind_detach
...
mi.register_integrator("prb_m2", lambda props: PRBM2Integrator(props))

...
class BasicPRBM2Integrator(RBIntegrator):
    r"""
        ...
        while loop(active):
            ...
            if not primal:
                with dr.resume_grad():
                    ...
                    Lr_detach = L * dr.replace_grad(1, detach_inv_bsdf_val_det * bsdf_val)

                    # Differentiable Monte Carlo estimate of all contributions
                    Lo = dr.sqrt((Le + Lr)*(Le + Lr_detach))*2.0*L_init
...
mi.register_integrator("prb_basic_m2", lambda props: BasicPRBM2Integrator(props))
